...
def get_default_config():
    ...
    if not osp.exists(user_config_file):
        copy_default_config_to_user(config_file,user_config_file)
    else:
        with open(user_config_file) as f:
            user_config = yaml.safe_load(f)
            if user_config['version'] !=config['version']:
                copy_default_config_to_user(config_file,user_config_file)
                logger.warn('Replacing older version user config: {}'.format(user_config_file))

    return config

def copy_default_config_to_user(config_file, user_config_file):
    try:
